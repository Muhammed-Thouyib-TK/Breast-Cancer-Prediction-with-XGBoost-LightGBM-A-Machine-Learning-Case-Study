   ...
   # Collect user input
    ...
    for feature in feature_names:
        if feature in unbounded_features:
            # Allow float values (like 0.017, 12.345 etc.)
            value = st.number_input(
                f"{feature}: ",
                min_value=0.0,
                max_value=10000.0,
                value=0.00000,
                step=0.00001,   # fine-grained step
                format="%.5f"   # force 5 decimal places
            )
        else:
            # Restrict to 0–1 features
            value = st.number_input(
                f"{feature}: ",
                min_value=0.0,
                max_value=1.0,
                value=0.00000,
                step=0.00001,   # allows precise decimals like 0.017
                format="%.5f"
            )
